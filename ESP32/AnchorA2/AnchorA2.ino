...
void setup() {
  ...
  Serial.println("### ANCHOR A2 START ###");
  Serial.print("EUI: "); Serial.println(anchor_addr);
  Serial.print("Antenna delay: "); Serial.println(Adelay);

  // NOTE: ESP32 ใช้ SPI.begin(SCK, MISO, MOSI, SS) หรือ SPI.begin(SCK, MISO, MOSI)
  ...
  // ใช้ mode เดิม (LOWPOWER) + ทำให้ log เสถียรด้วย filtering
...
void newDevice(DW1000Device *device) {
  Serial.print("[ANCHOR A2] Connected TAG: 0x");
...
void inactiveDevice(DW1000Device *device) {
  Serial.print("[ANCHOR A2] TAG inactive: 0x");
  Serial.println(device->getShortAddress(), HEX);
