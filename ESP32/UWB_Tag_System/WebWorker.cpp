...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .btn-danger { color: #f87171; border-color: #7f1d1d; background: #450a0a; }
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="stats-row">
      <div class="stat-item">
        <div class="stat-label">Tag 1 (Blue)</div>
        <div class="stat-val"><span id="t1_pos">-</span></div>
      </div>
      <div class="stat-item">
        <div class="stat-label">Tag 2 (Orange)</div>
        <div class="stat-val" style="color:var(--orange)"><span id="t2_pos">-</span></div>
      </div>
      <div class="stat-item">
  ...
  <div class="card">
    ...
    <div class="controls">
      ...
      <input id="cx" value="" placeholder="1.50">
      <input id="cy" value="" placeholder="1.00">
  ...
  <div class="card" style="border-color: rgba(249, 115, 22, 0.3);">
    ...
    <div class="controls">
      <span style="font-size:12px; color:#aaa; align-self:center;">REF:</span> 
      <input id="cx2" value="" placeholder="1.50">
      <input id="cy2" value="" placeholder="0.80">
      <div style="width:1px; height:24px; background:var(--border); margin:0 8px;"></div>
...
// Tag 1 Functions
function calp(){ 
    const x=document.getElementById('cx').value || "1.50"; 
    const y=document.getElementById('cy').value || "1.00"; 
    api(`/calp?x=${x}&y=${y}`); 
...
// Tag 2 Functions (Updated)
function calT2(){
    const x=document.getElementById('cx2').value || "1.50";
    const y=document.getElementById('cy2').value || "0.80";
    api(`/cal_t2?x=${x}&y=${y}`);
    document.getElementById('cal2-msg').textContent="CMD SENT...";
}
function saveT2(){
    api(`/save_t2`);
    document.getElementById('cal2-msg').textContent="SAVE CMD SENT...";
}
function resetT2(){
    api(`/reset_t2`);
    document.getElementById('cal2-msg').textContent="RESET CMD SENT...";
}
...
function drawMap(j) {
  ...
  const tx = (x) => ox + (x * sc);
  const ty = (y) => H - oy - (y * sc);

  ctx.clearRect(0,0,W,H);
  
  // Grid
  ctx.strokeStyle = '#27272a'; ctx.lineWidth = 1; ctx.strokeRect(tx(0), ty(FIELD_H), FIELD_W*sc, FIELD_H*sc);
  ctx.beginPath();
  ...
  if(j.anch_xy) {
    ...
    j.anch_xy.forEach((p, i) => {
      ...
      const isOk = (parseFloat(distStr) > 0);
      const labelY = (p[1] > FIELD_H/2) ? ay - 25 : ay + 25;
      
      ctx.fillStyle = isOk ? '#10b981' : '#71717a'; 
      ctx.fillText(`A${i+1} ${distStr}m`, ax, labelY);
  ...
  if(j.ok === 1 || j.x > -0.5) {
    ...
    ctx.beginPath(); ctx.arc(px, py, 12 + Math.sin(Date.now()/200)*2, 0, Math.PI*2); ctx.stroke();
    ctx.fillStyle = '#3b82f6'; ctx.beginPath(); ctx.arc(px, py, 7, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '12px monospace'; ctx.fillText(`Tag1`, px, py - 22);
  }

  // --- Draw Tag 2 (Orange) ---
  if(j.t2 && j.t2.ok === 1) {
    const px2 = tx(j.t2.x), py2 = ty(j.t2.y);
    ctx.strokeStyle = 'rgba(249, 115, 22, 0.4)'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(px2, py2, 12 + Math.sin(Date.now()/200+1)*2, 0, Math.PI*2); ctx.stroke();
    ctx.fillStyle = '#f97316'; ctx.beginPath(); ctx.arc(px2, py2, 7, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = '12px monospace'; ctx.fillText(`Tag2`, px2, py2 - 22);
  }
}

const ST_TXT = ["READY", "CALIBRATING...", "SUCCESS!", "FAILED", "RESET DONE"];
...
async function tick(){
  try {
    ...
    const t1El = document.getElementById('t1_pos');
    if(j.ok) t1El.textContent = `${j.x.toFixed(2)}, ${j.y.toFixed(2)}`;
    else t1El.textContent = "OFFLINE";

    const t2El = document.getElementById('t2_pos');
    if(j.t2 && j.t2.ok) t2El.textContent = `${j.t2.x.toFixed(2)}, ${j.t2.y.toFixed(2)}`;
    else t2El.textContent = "OFFLINE";

    document.getElementById('rmse').textContent = j.rmse.toFixed(3);
    
    const badge = document.getElementById('status-badge');
    badge.textContent = j.ok ? "ONLINE" : "SEARCHING";
    badge.className = j.ok ? "badge bg-ok" : "badge bg-bad";

    if(j.a) j.a.forEach((val, i) => { const el=document.getElementById('a'+(i+1)); if(el) el.textContent=val+" m"; });
...
static void handleCalCommon(bool multi) {
  ...
  if(valX == 0 && valY == 0) { valX = 1.50f; valY = 1.00f; }
...
void sendCommandToTag2(int cmd, float x, float y) {
  ...
  esp_now_send(tag2_mac, (uint8_t *) &myCmd, sizeof(myCmd)); 
...
// เพิ่ม Handler สำหรับปุ่ม Reset Tag 2
static void handleResetT2() {
  sendCommandToTag2(3, 0, 0); // Cmd 3 = Reset
...
void setupWeb() {
  ...
  // Endpoints สำหรับ Tag 2
  server.on("/cal_t2", handleCalT2);
  server.on("/save_t2", handleSaveT2);
  server.on("/reset_t2", handleResetT2); // เพิ่มบรรทัดนี้
